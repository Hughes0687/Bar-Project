...
# # # # # # # # # # # # # # # # # # # # # #
  ...
  describe '#happy_hour?' do
    it "knows when it is happy hour (3:00pm to 4:00pm)" do
      # TODO: CONTROL TIME
      Time.stub(:now).and_return(Time.parse('3:45 pm'))
      expect(@bar.happy_hour?).to eq(true)
    end

    it "is not happy hour otherwise" do
      # TODO: CONTROL TIME
      Time.stub(:now).and_return(Time.parse('4:00 pm'))
  ...
  context "During normal hours" do
    # TODO: WRITE TESTS TO ENSURE BAR KNOWS NOT TO DISCOUNT
      it "doesn't have a discount" do
      @bar.happy_discount = 0.25
      Time.stub(:now).and_return(Time.parse('June 4 2014 12:45 pm'))
      expect(@bar.happy_discount).to eq(0)
    end
  end
  ...
  context "During happy hours" do
   ...
   it "does have a discount" do
      @bar.happy_discount = 0.25
      Time.stub(:now).and_return(Time.parse('June 4 2014 3:00 pm'))
